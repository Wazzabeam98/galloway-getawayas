...
export default async function JobMessagesPage() {
    ...
    const threads = await listEnquiryThreadsFor(admin, user.id);
    ...
    return (
        <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8 pb-24">
            ...
            <p className="mt-1.5 text-sm text-slate-500">Your job threads — where what was agreed is written down.</p>

            {threads.length === 0 ? (
                <div className="mt-8 rounded-2xl border border-slate-200 bg-slate-50 p-6">
                    <p className="font-semibold text-slate-900">No job messages yet.</p>
                    <p className="text-sm text-slate-600 mt-1.5">Once a job is accepted, you can message about it from here.</p>
                </div>
            ) : (
                <ul className="mt-6 divide-y divide-slate-100 rounded-2xl border border-slate-200 bg-white overflow-hidden">
                    {threads.map((t) => (
                        <li key={t.enquiryId}>
                            <Link href={`/messages/enquiry/${t.enquiryId}`} className="flex items-center gap-3 px-4 py-3.5 hover:bg-slate-50 transition">
                                ...
                                <div className="min-w-0 flex-1">
                                    <div className="flex items-center gap-2">
                                        ...
                                        {t.cancelled && <span className="flex-none text-[10.5px] font-bold uppercase tracking-wide text-rose-700 bg-rose-50 border border-rose-200 rounded-full px-1.5 py-0.5">Cancelled</span>}
                                    ...
                                    <div className={`text-[13px] truncate mt-0.5 ${t.unread ? 'text-slate-700 font-medium' : 'text-slate-500'}`}>
                                        {t.lastBody || <span className="italic text-slate-400">{t.summary}</span>}
                                    </div>
                                </div>
