...
export default function Hero() {
  ...
  if (dateTab === 'dates' && dateRange.startDate && dateRange.endDate) {
    ...
    if (selectedMonths.length > 0) {
      ...
      whenSummary = `A ${stayDuration} in ${format(sorted[0], 'MMM')}${sorted.length > 1 ? '...' : ''}`;
    } else {
      whenSummary = `Any ${stayDuration}`;
    }
  }

  // Generate 12 rolling months starting from today
  const futureMonths = Array.from({ length: 12 }, (_, i) => addMonths(new Date(), i));

  return (
    <div className="relative w-full h-[500px] flex items-center justify-center bg-stone-900 text-white overflow-visible" ref={heroRef}>
      <div 
      ...
      <div className="relative z-10 text-center max-w-5xl px-4 flex flex-col items-center">
        ...
        <div className="w-full max-w-3xl bg-white rounded-full p-2 shadow-2xl text-stone-800 flex items-center relative border border-stone-100">
          ...
          <div 
            ...
            className={`flex-1 px-5 py-2.5 text-left rounded-full transition cursor-pointer ${
              activePopover === 'where' ? 'bg-white shadow-md' : 'hover:bg-stone-100'
            }`}
          >
            <label className="block text-[10px] font-bold tracking-wider uppercase text-stone-700 cursor-pointer">Where</label>
            <select
              value={location}
          ...
          >
            <label className="block text-[10px] font-bold tracking-wider uppercase text-stone-700 cursor-pointer">When</label>
          ...
          >
            <label className="block text-[10px] font-bold tracking-wider uppercase text-stone-700 cursor-pointer">Who</label>
          ...
          {/* =========================================
             ...
             ========================================= */}
          {activePopover === 'when' && (
            <div className="absolute left-1/2 -translate-x-1/2 top-full mt-3 bg-white rounded-3xl p-6 shadow-2xl border border-stone-100 text-stone-900 z-50 min-w-[650px]">
              {/* Tab Switcher */}
              <div className="flex justify-center mb-6">
                <div className="bg-stone-100 p-1 rounded-full flex gap-1 border border-stone-200">
                  <button 
                    ...
                    className={`px-8 py-2 rounded-full text-sm font-semibold transition ${dateTab === 'dates' ? 'bg-white shadow text-stone-900' : 'text-stone-600'}`}
                  >
                    Dates
                  </button>
                  <button 
                    ...
                    className={`px-8 py-2 rounded-full text-sm font-semibold transition ${dateTab === 'flexible' ? 'bg-white shadow text-stone-900' : 'text-stone-600'}`}
                  >
                    Flexible
                  </button>
                </div>
                      ...
                      {durations.map((duration) => (
                        <button
                          ...
                          className={`px-5 py-2 rounded-full border text-sm font-medium capitalize transition ${
                            stayDuration === duration ? 'border-stone-900 bg-stone-900 text-white' : 'border-stone-300 text-stone-700 hover:border-stone-900'
                          }`}
                        >
                          {duration}
                        </button>
                      ...
                      {futureMonths.map((month, i) => {
                        ...
                        return (
                          <button
                            ...
                            className={`p-3 rounded-2xl border transition flex flex-col items-center justify-center ${
                              isSelected ? 'border-emerald-700 bg-emerald-50 text-emerald-900 ring-2 ring-emerald-700' : 'border-stone-200 hover:border-stone-400'
                            }`}
                          >
          ...
          {/* =========================================
             ...
             ========================================= */}
          {activePopover === 'who' && (
            <div className="absolute right-0 top-full mt-3 w-80 bg-white rounded-3xl p-6 shadow-2xl border border-stone-100 text-stone-900 z-50">
              <div className="flex items-center justify-between py-3 border-b border-stone-100">
                ...
                <div className="flex items-center gap-3">
                  <button type="button" onClick={() => setAdults(Math.max(0, adults - 1))} className="w-8 h-8 rounded-full border border-stone-300 flex items-center justify-center text-stone-600 hover:border-stone-800 disabled:opacity-30">−</button>
                  <span className="w-4 text-center text-sm font-medium">{adults}</span>
                  <button type="button" onClick={() => setAdults(adults + 1)} className="w-8 h-8 rounded-full border border-stone-300 flex items-center justify-center text-stone-600 hover:border-stone-800">+</button>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between py-3 border-b border-stone-100">
                ...
                <div className="flex items-center gap-3">
                  <button type="button" onClick={() => setChildren(Math.max(0, children - 1))} className="w-8 h-8 rounded-full border border-stone-300 flex items-center justify-center text-stone-600 hover:border-stone-800 disabled:opacity-30">−</button>
                  <span className="w-4 text-center text-sm font-medium">{children}</span>
                  <button type="button" onClick={() => { setChildren(children + 1); if (adults === 0) setAdults(1); }} className="w-8 h-8 rounded-full border border-stone-300 flex items-center justify-center text-stone-600 hover:border-stone-800">+</button>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between py-3 border-b border-stone-100">
                ...
                <div className="flex items-center gap-3">
                  <button type="button" onClick={() => setInfants(Math.max(0, infants - 1))} className="w-8 h-8 rounded-full border border-stone-300 flex items-center justify-center text-stone-600 hover:border-stone-800 disabled:opacity-30">−</button>
                  <span className="w-4 text-center text-sm font-medium">{infants}</span>
                  <button type="button" onClick={() => { setInfants(infants + 1); if (adults === 0) setAdults(1); }} className="w-8 h-8 rounded-full border border-stone-300 flex items-center justify-center text-stone-600 hover:border-stone-800">+</button>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between pt-3">
                ...
                <div className="flex items-center gap-3">
                  <button type="button" onClick={() => setPets(Math.max(0, pets - 1))} className="w-8 h-8 rounded-full border border-stone-300 flex items-center justify-center text-stone-600 hover:border-stone-800 disabled:opacity-30">−</button>
                  <span className="w-4 text-center text-sm font-medium">{pets}</span>
                  <button type="button" onClick={() => { setPets(pets + 1); if (adults === 0) setAdults(1); }} className="w-8 h-8 rounded-full border border-stone-300 flex items-center justify-center text-stone-600 hover:border-stone-800">+</button>
                </div>
              </div>
            </div>
          )}
