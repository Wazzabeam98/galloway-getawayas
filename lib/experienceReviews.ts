...
//
// Reused from feat/experience-reviews. The cottage side stores rating aggregates
// on the listings row and keeps them in step with a trigger. service_providers
// deliberately keeps no counters — a stored count is one missed write from being
// wrong — so this computes the count and average on read, the same way the
// marketplace computes bookingsCount.
//
// The number a listing SHOWS follows the cottage rule (lib/reviews): a handful of
// reviews is not a rating, so the average is withheld until MIN_PUBLIC_REVIEWS of
// them exist. Below that the individual reviews still show — words, not a score.
//
// This always returns a block (never null), so the listing can render an honest
// "No reviews yet" in place rather than dropping the section — a missing section
// reads as unfinished, an empty one reads as new.
//
...
export interface ExperienceReviewsBlock {
    ...
    // Whether the current viewer is the provider (parity with the cottage's host).
    canReply: boolean;
...
export async function loadExperienceReviews(
...
): Promise<ExperienceReviewsBlock> {
    ...
    // The provider owner — read even with no reviews, so the empty state and the
    // reply parity both have a name to use.
    ...
    const canReply = !!viewerId && !!ownerId && viewerId === ownerId;

    if (list.length === 0) {
        return { count: 0, avg: null, items: [], canReply, providerFirstName };
    }
